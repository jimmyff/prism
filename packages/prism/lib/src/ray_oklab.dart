...
base class RayOklab extends Ray {
  ...
  const RayOklab.empty() : l = 0.0, a = 0.0, b = 0.0, _opacity = 0.0;

  /// Creates an Oklab color from a JSON value.
  /// 
  /// The JSON should be a Map with 'l', 'a', 'b', and 'opacity' keys.
  ...
  RayOklab lerp(Ray other, double t) {
    if (t < 0.0 || t > 1.0) {
      throw ArgumentError.value(t, 't', 'Interpolation factor must be between 0.0 and 1.0');
    }
  ...
  RayRgb toRgb() {
    ...
    final linearR = 4.0767416621 * lCubed - 3.3077115913 * mCubed + 0.2309699292 * sCubed;
    final linearG = -1.2684380046 * lCubed + 2.6097574011 * mCubed - 0.3413193965 * sCubed;
    final linearB = -0.0041960863 * lCubed - 0.7034186147 * mCubed + 1.7076147010 * sCubed;
  ...
  static RayOklab _fromLinearRgb(double r, double g, double b, double opacity) {
    ...
    final lms1 = 0.4122214708 * linearR + 0.5363325363 * linearG + 0.0514459929 * linearB;
    final lms2 = 0.2119034982 * linearR + 0.6806995451 * linearG + 0.1073969566 * linearB;
    final lms3 = 0.0883024619 * linearR + 0.2817188376 * linearG + 0.6299787005 * linearB;
    ...
    // Transform to Oklab
    final labL = 0.2104542553 * lmsCbrt1 + 0.7936177850 * lmsCbrt2 - 0.0040720468 * lmsCbrt3;
    final labA = 1.9779984951 * lmsCbrt1 - 2.4285922050 * lmsCbrt2 + 0.4505937099 * lmsCbrt3;
    final labB = 0.0259040371 * lmsCbrt1 + 0.7827717662 * lmsCbrt2 - 0.8086757660 * lmsCbrt3;
