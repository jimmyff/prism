...
/// Example:
/// ```dart
/// final scheme = RayScheme.fromRay(Ray.fromHex('#2196F3'));
...
class RayScheme {
  ...
  /// Example:
  /// ```dart
  /// final blueScheme = RayScheme.fromRay(Ray.fromHex('#2196F3'));
  /// final redScheme = RayScheme.fromRay(Ray.fromHex('#F44336'));
  /// ```
  factory RayScheme.fromRay(Ray ray) {
    final luminance = ray.computeLuminance();
    final isDark = luminance < _darkThreshold;

    const black = Ray(0xFF000000);
    const white = Ray(0xFFFFFFFF);
    ...
    if (luminance < _targetLightLuminance) {
      ...
      surfaceLight = ray.lerp(white, t);
    } else {
      final t = _getLerpAmount(
          _targetLightLuminance, luminance, 0.0); // Mix with black
      surfaceLight = ray.lerp(black, t);
    }
