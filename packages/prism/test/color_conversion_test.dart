  ...
  group('Color Model Conversions', () {
    ...
    group('RGB → HSL Conversions', () {
      ...
      test('RGB green to HSL', () {
        final rgb = RayRgb(red: 0, green: 255, blue: 0);
        final hsl = rgb.toHsl();
      ...
      test('RGB blue to HSL', () {
        final rgb = RayRgb(red: 0, green: 0, blue: 255);
        final hsl = rgb.toHsl();
      ...
      test('RGB black to HSL', () {
        final rgb = RayRgb(red: 0, green: 0, blue: 0);
        final hsl = rgb.toHsl();
      ...
      test('RGB white to HSL', () {
        final rgb = RayRgb(red: 255, green: 255, blue: 255);
        final hsl = rgb.toHsl();
      ...
      test('RGB gray to HSL', () {
        final rgb = RayRgb(red: 128, green: 128, blue: 128);
        final hsl = rgb.toHsl();
        
        expect(hsl.hue, closeTo(0, precisionTolerance));
        expect(hsl.saturation, closeTo(0, precisionTolerance));
        expect(hsl.lightness, closeTo(0.502, componentTolerance)); // 128/255 ≈ 0.502
      });

      test('RGB cyan to HSL', () {
        final rgb = RayRgb(red: 0, green: 255, blue: 255);
        final hsl = rgb.toHsl();
      ...
      test('RGB magenta to HSL', () {
        final rgb = RayRgb(red: 255, green: 0, blue: 255);
        final hsl = rgb.toHsl();
      ...
      test('RGB yellow to HSL', () {
        final rgb = RayRgb(red: 255, green: 255, blue: 0);
        final hsl = rgb.toHsl();
      ...
      test('RGB to HSL preserves opacity', () {
        final rgb = RayRgb(red: 255, green: 0, blue: 0, alpha: 128);
        final hsl = rgb.toHsl();
      ...
      test('RGB orange to HSL', () {
        // Orange-ish color
        final rgb = RayRgb(red: 255, green: 165, blue: 0);
        final hsl = rgb.toHsl();
        
        expect(hsl.hue, closeTo(38.8, hueTolerance)); // Expected hue for this orange
    ...
    group('HSL → RGB Conversions', () {
      ...
      test('HSL to RGB preserves opacity', () {
        final hsl = RayHsl(hue: 120, saturation: 1.0, lightness: 0.5, opacity: 0.502);
        final rgb = hsl.toRgb();
    ...
    group('RGB → Oklab Conversions', () {
      test('RGB red to Oklab', () {
        ...
        expect(oklab.b, greaterThan(0.0)); // Red should have positive b (yellow component)
    ...
    group('RGB ↔ HSL Round-trip Conversions', () {
      final testColors = [
        RayRgb(red: 255, green: 0, blue: 0),       // Red
        RayRgb(red: 0, green: 255, blue: 0),       // Green
        RayRgb(red: 0, green: 0, blue: 255),       // Blue
        RayRgb(red: 255, green: 255, blue: 0),     // Yellow
        RayRgb(red: 255, green: 0, blue: 255),     // Magenta
        RayRgb(red: 0, green: 255, blue: 255),     // Cyan
        RayRgb(red: 128, green: 128, blue: 128),   // Gray
        RayRgb(red: 255, green: 165, blue: 0),     // Orange
        RayRgb(red: 128, green: 0, blue: 128),     // Purple
        RayRgb(red: 255, green: 192, blue: 203),   // Pink
        RayRgb(red: 0, green: 128, blue: 0),       // Dark Green
        RayRgb(red: 139, green: 69, blue: 19),     // Brown
      ];
      ...
      for (int i = 0; i < testHslColors.length; i++) {
        test('HSL→RGB→HSL round-trip preserves color ${i + 1}', () {
          ...
          expect(convertedHsl.saturation, closeTo(originalHsl.saturation, componentTolerance));
          expect(convertedHsl.lightness, closeTo(originalHsl.lightness, componentTolerance));
          expect(convertedHsl.opacity, closeTo(originalHsl.opacity, componentTolerance));
        });
      }
    });
    ...
    group('RGB ↔ Oklab Round-trip Conversions', () {
      final testColors = [
        ...
        RayRgb(red: 0, green: 255, blue: 0),       // Green
        RayRgb(red: 0, green: 0, blue: 255),       // Blue
        RayRgb(red: 255, green: 255, blue: 255),   // White
        RayRgb(red: 0, green: 0, blue: 0),         // Black
        RayRgb(red: 128, green: 128, blue: 128),   // Gray
        RayRgb(red: 255, green: 165, blue: 0),     // Orange
        RayRgb(red: 128, green: 0, blue: 128),     // Purple
      ];
    ...
    group('HSL ↔ Oklab Round-trip Conversions', () {
      ...
      for (int i = 0; i < testColors.length; i++) {
        test('HSL→Oklab→HSL round-trip preserves approximate color ${i + 1}', () {
          ...
          expect((backToHsl.saturation - originalHsl.saturation).abs(), lessThan(0.15));
          expect((backToHsl.lightness - originalHsl.lightness).abs(), lessThan(0.15));
    ...
    group('Multi-space Conversion Chains', () {
      test('RGB→HSL→Oklab→RGB preserves approximate color', () {
        final originalRgb = RayRgb(red: 128, green: 64, blue: 192);
        final hsl = originalRgb.toHsl();
        final oklab = hsl.toOklab();
      ...
      test('RGB→Oklab→HSL→RGB preserves approximate color', () {
        final originalRgb = RayRgb(red: 200, green: 100, blue: 50);
        final oklab = originalRgb.toOklab();
        final hsl = oklab.toHsl();
    ...
    group('Opacity Preservation Across All Conversions', () {
      test('all conversions preserve opacity values', () {
        ...
        for (final opacity in opacities) {
          final rgb = RayRgb(red: 128, green: 64, blue: 192, alpha: (opacity * 255).round());
    ...
    group('Edge Cases and Special Color Behaviors', () {
      test('very dark RGB colors convert correctly', () {
        final rgb = RayRgb(red: 1, green: 0, blue: 0);
        final hsl = rgb.toHsl();
        final backToRgb = hsl.toRgb();
      ...
      test('very bright RGB colors convert correctly', () {
        final rgb = RayRgb(red: 254, green: 255, blue: 254);
        final hsl = rgb.toHsl();
        final backToRgb = hsl.toRgb();
      ...
      test('transparent colors preserve transparency', () {
        final rgbTransparent = RayRgb(red: 255, green: 0, blue: 0, alpha: 0);
      ...
      test('HSL hue normalization works correctly', () {
        ...
        for (int i = 0; i < hues.length; i++) {
          final hsl = RayHsl(hue: hues[i].toDouble(), saturation: 1.0, lightness: 0.5);
          final rgb = hsl.toRgb();
          
          final normalizedHsl = RayHsl(hue: expectedNormalized[i].toDouble(), saturation: 1.0, lightness: 0.5);
    ...
    group('Conversion Performance', () {
      test('RGB→HSL conversion is reasonably fast', () {
        final rgb = RayRgb(red: 128, green: 64, blue: 192);
        ...
        stopwatch.stop();
        expect(stopwatch.elapsedMicroseconds, lessThan(10000)); // Less than 10ms for 1000 conversions
      });
      ...
      test('RGB→Oklab conversion is reasonably fast', () {
        ...
        stopwatch.stop();
        expect(stopwatch.elapsedMicroseconds, lessThan(50000)); // Less than 50ms for 1000 conversions
      });
