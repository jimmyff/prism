...
void main() {
  group('RayHsl Constructors', () {
    test('creates HSL color with named parameters', () {
      ...
      expect(hsl.colorModel, equals(ColorModel.hsl));
    });

    test('creates HSL color with opacity', () {
      final hsl = RayHsl(hue: 240, saturation: 1.0, lightness: 0.5, opacity: 0.7);
    ...
    test('clamps saturation and lightness values', () {
      final hsl = RayHsl(hue: 0, saturation: 1.5, lightness: -0.1, opacity: 1.2);
  ...
  group('RayHsl Methods', () {
    test('withHue creates new HSL with different hue', () {
      final original = RayHsl(hue: 0, saturation: 0.8, lightness: 0.6, opacity: 0.7);
      ...
      expect(original.hue, closeTo(0, precisionTolerance)); // Original unchanged
    });
    ...
    test('inverse creates complementary HSL color', () {
      final original = RayHsl(hue: 60, saturation: 0.8, lightness: 0.3, opacity: 0.7);
      final inverted = original.inverse;
      
      expect(inverted.hue, closeTo(240, precisionTolerance)); // 60 + 180
      expect(inverted.saturation, closeTo(0.2, precisionTolerance)); // 1.0 - 0.8
  ...
  group('RayHsl Interpolation', () {
    ...
    test('lerp interpolates opacity', () {
      final opaque = RayHsl(hue: 120, saturation: 0.5, lightness: 0.5, opacity: 1.0);
      final transparent = RayHsl(hue: 120, saturation: 0.5, lightness: 0.5, opacity: 0.0);
    ...
    test('lerp with RGB color converts and interpolates', () {
      ...
      final rgb = RayRgb(red: 255, green: 0, blue: 0);
      final result = hsl.lerp(rgb, 0.5);
      
      expect(result.colorModel, equals(ColorModel.hsl));
  ...
  group('RayHsl Conversions', () {
    ...
    test('toJson creates correct map', () {
      final hsl = RayHsl(hue: 270, saturation: 0.4, lightness: 0.8, opacity: 0.6);
  ...
  group('RayHsl Equality and Hashing', () {
    test('equals operator works correctly', () {
      final hsl1 = RayHsl(hue: 45, saturation: 0.7, lightness: 0.6, opacity: 0.8);
      final hsl2 = RayHsl(hue: 45, saturation: 0.7, lightness: 0.6, opacity: 0.8);
      final hsl3 = RayHsl(hue: 46, saturation: 0.7, lightness: 0.6, opacity: 0.8);
    ...
    test('handles floating point precision in equality', () {
      ...
      final hsl2 = RayHsl(hue: 120.0000000001, saturation: 0.5000000001, lightness: 0.5);
  ...
  group('RayHsl String Representation', () {
    ...
    test('toString formats correctly with opacity', () {
      final hsl = RayHsl(hue: 0, saturation: 1.0, lightness: 0.5, opacity: 0.75);
      final str = hsl.toString();
  ...
  group('RayHsl Luminance', () {
    test('computeLuminance works for pure colors', () {
      final red = RayHsl(hue: 0, saturation: 1.0, lightness: 0.5);
      final green = RayHsl(hue: 120, saturation: 1.0, lightness: 0.5);
      final blue = RayHsl(hue: 240, saturation: 1.0, lightness: 0.5);
      
      final redLum = red.computeLuminance();
      final greenLum = green.computeLuminance();
      final blueLum = blue.computeLuminance();
    ...
    test('computeLuminance for grayscale matches lightness', () {
      ...
      final lum25 = gray25.computeLuminance();
      final lum50 = gray50.computeLuminance();
      final lum75 = gray75.computeLuminance();
  ...
  group('RayHsl Difference Functions', () {
    ...
    test('hueDifference handles hue wraparound correctly', () {
      ...
      expect(almostRed1.hueDifference(almostRed2), closeTo(20.0, precisionTolerance));
      
      // 10° to 350° should be -20°
      expect(almostRed2.hueDifference(almostRed1), closeTo(-20.0, precisionTolerance));
    });
    ...
    test('saturationDifference calculates signed saturation difference', () {
      ...
      expect(vivid.saturationDifference(pastel), closeTo(-0.8, precisionTolerance));
      
      // Pastel to vivid: +0.8 (vivid is more saturated)
      expect(pastel.saturationDifference(vivid), closeTo(0.8, precisionTolerance));
      
      // Medium to pastel: -0.3
      expect(medium.saturationDifference(pastel), closeTo(-0.3, precisionTolerance));
    });
    ...
    test('lightnessDifference calculates signed lightness difference', () {
      ...
      expect(dark.lightnessDifference(bright), closeTo(0.6, precisionTolerance));
      
      // Bright to dark: -0.6 (dark is darker)
      expect(bright.lightnessDifference(dark), closeTo(-0.6, precisionTolerance));
      
      // Medium to dark: -0.3
      expect(medium.lightnessDifference(dark), closeTo(-0.3, precisionTolerance));
    });
    ...
    test('difference functions handle identical colors', () {
      ...
      expect(color1.saturationDifference(color2), closeTo(0.0, precisionTolerance));
      expect(color1.lightnessDifference(color2), closeTo(0.0, precisionTolerance));
    });
  });
  ...
  group('RayHsl Distance Functions', () {
    test('hueDistance calculates absolute hue distance', () {
      ...
      expect(red.hueDistance(blue), closeTo(120.0, precisionTolerance)); // Shorter path
    ...
    test('hueDistance handles wraparound correctly', () {
      ...
      expect(almostRed1.hueDistance(almostRed2), closeTo(20.0, precisionTolerance));
      expect(almostRed2.hueDistance(almostRed1), closeTo(20.0, precisionTolerance));
    });
    ...
    test('saturationDistance calculates absolute saturation distance', () {
      ...
      // All distances should be positive
      expect(vivid.saturationDistance(pastel), closeTo(0.8, precisionTolerance));
      expect(pastel.saturationDistance(vivid), closeTo(0.8, precisionTolerance));
      expect(medium.saturationDistance(pastel), closeTo(0.3, precisionTolerance));
      
      // Distance is symmetric
      expect(vivid.saturationDistance(medium), equals(medium.saturationDistance(vivid)));
    });
    ...
    test('lightnessDistance calculates absolute lightness distance', () {
      ...
      // Distance is symmetric
      expect(dark.lightnessDistance(medium), equals(medium.lightnessDistance(dark)));
    });
    ...
    test('distance functions handle identical colors', () {
      ...
      expect(color1.saturationDistance(color2), closeTo(0.0, precisionTolerance));
      expect(color1.lightnessDistance(color2), closeTo(0.0, precisionTolerance));
    });
    ...
    test('distance functions are absolute values of difference functions', () {
      ...
      expect(color1.hueDistance(color2), equals(color1.hueDifference(color2).abs()));
      expect(color1.saturationDistance(color2), equals(color1.saturationDifference(color2).abs()));
      expect(color1.lightnessDistance(color2), equals(color1.lightnessDifference(color2).abs()));
    });
